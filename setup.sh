...
ansible-pull -U https://github.com/viasoftkorp/KorpSetupLinux.git bootstrap-playbook.yml --limit localhost --vault-id /etc/korp/ansible/.vault_key --extra-vars='{"token": "'$token'", "gateway_url":"'$gateway_url'", "apps":['$apps']}' --tags=default-setup,install-apps
if [ $? != 0 ]
then
